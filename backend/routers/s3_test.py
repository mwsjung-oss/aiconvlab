"""GET /test-s3 — 운영용 S3 연결 점검 (플래그·admin 인증 후 업로드·즉시 삭제).

``ENABLE_S3_TEST_ENDPOINT=true`` 일 때만 노출되며, 그 외에는 404 처리합니다."""
...
@router.get(
    ...
    description=(
        ...
        "활성화: `ENABLE_S3_TEST_ENDPOINT=true`, **Bearer JWT** (admin 또는 master 역할), "
        "`AWS_*` / `STORAGE_*` 자격 증명 및 리전 필요."
    ),
    responses={
        200: {"description": "성공 — `upload_ok` / `delete_ok` 포함"},
        400: {"description": "필수 환경 변수 누락 — `detail.missing_environment_variables`"},
        401: {"description": "Bearer 미제공 또는 토큰 무효"},
        403: {"description": "admin · master 가 아님"},
...
def test_s3_upload(
    _: User = Depends(get_current_admin_operator),
) -> dict[str, Any]:
    """boto3로 소형 테스트 객체 업로드 직후 동일 객체를 삭제하고 JSON으로 결과를 반환합니다."""
