...
from brief_file_extract import extract_text_from_bytes
from brief_refine_feedback import refine_brief_from_user_message
from brief_title_suggest import suggest_title_from_content
from project_intelligence import analyze_brief, build_data_prep_guide, build_intelligence_json
from models import (
    Announcement,
    Assignment,
    Course,
...
class DataPrepGuideBody(BaseModel):
    """등록·분석된 브리프 기준 데이터 준비 가이드."""
    ...
    analysis: dict[str, Any] | None = Field(
        default=None,
        description="클라이언트에 캐시된 analyze 결과(있으면 재사용)",
    )
...
def suggest_project_title_endpoint(
    body: SuggestTitleBody,
    current_user: User = Depends(get_current_approved_member),
) -> dict[str, Any]:
    del current_user
    result = suggest_title_from_content(body.content)
    if not result.get("ok"):
        raise HTTPException(
            status_code=400, detail=result.get("error", "제목 추출 실패")
        )
    return result


@router.post("/projects/extract-file-text")
async def extract_project_brief_file_text(
    file: UploadFile = File(...),
    current_user: User = Depends(get_current_approved_member),
) -> dict[str, Any]:
    """브리프 본문에 넣을 평문 추출 (Word/Excel/PPT/PDF 등)."""
    del current_user
    raw = await file.read()
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail=f"파일에서 텍스트를 추출할 수 없습니다: {e!s}",
        ) from e
    stripped = (text or "").strip()
    if not stripped:
        raise HTTPException(status_code=400, detail="추출된 텍스트가 비어 있습니다.")
    return {"ok": True, "text": stripped, "filename": name}
...
@router.post("/projects/refine-brief")
def refine_project_brief_endpoint(
    body: BriefRefineBody,
    current_user: User = Depends(get_current_approved_member),
) -> dict[str, Any]:
    del current_user
    result = refine_brief_from_user_message(
        title=body.title,
        content=body.content,
        user_message=body.user_message,
    )
    if not result.get("ok"):
        raise HTTPException(status_code=400, detail=result.get("error", "반영 실패"))
    return result


@router.post("/projects/data-prep-guide")
def data_prep_guide_endpoint(
    body: DataPrepGuideBody,
    current_user: User = Depends(get_current_approved_member),
) -> dict[str, Any]:
    del current_user
    result = build_data_prep_guide(
        source_type=body.source_type,
        title=body.title,
        content=body.content,
        cached_analysis=body.analysis,
    )
    if not result.get("ok"):
        raise HTTPException(status_code=400, detail=result.get("error", "가이드 생성 실패"))
    return result
