...
export default function App() {
  ...
  }, [isAuthenticated, loadMonitoring, loadJobs, loadArtifacts]);
  ...
  const navRowPortal = [
    ...
    ...(isPrivilegedRole(user?.role) ? [{ id: "artifacts", label: "Artifacts" }] : []),
    ...(isPrivilegedRole(user?.role) ? [{ id: "monitor", label: "System" }] : []),
    ...(isPrivilegedRole(user?.role) ? [{ id: "admin", label: "관리자" }] : []),
  ];
  ...
  return (
    <div className="app">
      ...
      {!isAuthenticated ? (
        <nav className="top-nav top-nav--auth">
          <button
            type="button"
            className={
              currentPage === "home" ? "nav-tab nav-tab-active" : "nav-tab"
            }
          ...
          <button
            type="button"
            className={
              currentPage === "dashboard" ? "nav-tab nav-tab-active" : "nav-tab"
            }
        ...
        <div className="top-nav-wrap">
          <div className="top-nav-row top-nav-row--portal">
            <span className="top-nav-row-label">포털 · 관리</span>
            <div className="top-nav-row-btns">
              {navRowPortal.map((item) => (
                <button
                  key={item.id}
                  type="button"
                  className={
                    currentPage === item.id ? "nav-tab nav-tab-active" : "nav-tab"
                  }
          ...
          <div className="top-nav-row top-nav-row--lab">
            <span className="top-nav-row-label">실습 절차</span>
            <div className="top-nav-row-btns">
              {navRowLab.map((item) => (
                <button
                  key={item.id}
          ...
          <div className="top-nav-row top-nav-row--ai">
            <span className="top-nav-row-label">AI 실습 코파일럿</span>
            <div className="top-nav-row-btns">
              {navRowAi.map((item) => (
                <button
                  key={item.preset}
                  type="button"
                  className={
                    currentPage === "aichat" && aiChatPreset === item.preset
                      ? "nav-tab nav-tab-active nav-tab--ai"
                      : "nav-tab nav-tab--ai"
                  }
                  onClick={() => {
                    setAiChatPreset(item.preset);
      ...
      {isAuthenticated && currentPage === "datasets_catalog" && (
        <DatasetsPage
      ...
      {isAuthenticated && currentPage === "preview" && (
        <PreviewPage
          datasets={datasets}
      ...
      {isAuthenticated && currentPage === "aichat" && (
        <AiChatPage
          key={aiChatPreset}
      ...
      {isAuthenticated && currentPage === "train" && (
        <TrainPage
          datasets={datasets}
          selectedFile={selectedFile}
      ...
      {isAuthenticated && currentPage === "predict" && (
        <PredictionPage
          datasets={datasets}
          models={models}
      ...
      {isAuthenticated && currentPage === "results" && (
        <ResultsPage
          trainResult={trainResult}
          plotUrl={plotUrl}
      ...
      {isAuthenticated && currentPage === "experiments" && (
        <ExperimentsPlatformPage />
      )}

      {isAuthenticated && currentPage === "notebook" && <NotebookPage />}

      {isAuthenticated && currentPage === "reports" && (
      ...
      {isAuthenticated && currentPage === "jobs" && (
        <JobsPage
          jobs={jobs}
          onRefresh={loadJobs}
      ...
      {architectureOpen && (
        <div className="arch-overlay" onClick={() => setArchitectureOpen(false)}>
          <div className="arch-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="hint">
              End-to-end flow between local UI, API server, and GPU execution environment.
            </p>
