...
const AI_PROVIDER_STORAGE_KEY = "ailab_ai_provider";

function readStoredProvider() {
  try {
    const v = localStorage.getItem(AI_PROVIDER_STORAGE_KEY);
    if (["openai", "gemini", "ollama", "local"].includes(v)) return v;
  } catch {
    /* ignore */
  }
  return "openai";
...
export default function AiChatPage({ onAfterTool, labPreset = "overview" }) {
  ...
  const [aiProvider, setAiProvider] = useState(readStoredProvider);
        ...
        <div className="ai-chat-provider-box">
          <label className="ai-chat-provider-label">
            ...
            >
              ...
              <option value="local">로컬만 (슬래시·키워드)</option>
            </select>
          </label>
        </div>
      </div>
