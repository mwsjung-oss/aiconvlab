...
def _cors_middleware_params() -> dict:
    """로컬 Vite 등 개발 오리진과 원격 프론트 배포 오리진을 CORS_ORIGINS 로 지정합니다.
    비어 있으면 localhost:5174 / 127.0.0.1:5174 만 허용합니다.
    allow_origins=['*'] 와 allow_credentials=True 는 브라우저 규격상 함께 쓸 수 없어,
    CORS_ORIGINS=* 일 때는 credentials 를 끕니다.
    """
